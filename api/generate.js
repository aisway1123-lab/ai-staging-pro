...
export default async function handler(req, res) {
  ...
  if (action === 'stageImage') {
    ...
    const ROOM_PROMPTS = {
      bedroom: "Room type: Bedroom. Primarily used for sleeping. A bed must be the main focal point. Replace all existing furniture and non-structural cabinetry with a new, realistic layout. Integrate secondary pieces like nightstands or a small vanity that best fit the room's proportions and circulation. Do not add any built-in or wall-embedded storage unless it already exists in the original image.",
    ...
    const F_Negative = `The number of walls, openings, windows, and doorways must exactly match the original image — no additions, no removals, no relocations. Do not create any new opening or door-shaped form on any wall surface. Do not include: people, animals, text, watermark, fantasy, cartoon. Do not add curtains, drapes, or window treatments to walls, doorways, or openings that are not windows.`;
